...
PLATFORMS=(
    # xcodebuild destination            XCFramework folder name          ARCHS
    "generic/platform=macOS"           "macos"                          "arm64 x86_64"
    "generic/platform=iOS Simulator"   "ios-simulator"                  "arm64 x86_64"
...
for ((i = 0; i < ${#PLATFORMS[@]}; i += 3)); do
    ...
    OBJECT_FILES="$DERIVED_DATA_PATH/Build/Intermediates.noindex/swift-syntax.build/$CONFIGURATION*/*.build/Objects-normal/*/*.o"
    if ls $OBJECT_FILES 1> /dev/null 2>&1; then
        ar -crs "$LIBRARY_PATH" $OBJECT_FILES
    else
        echo "Error: No object files found for $ARCHS on $XCODEBUILD_PLATFORM_NAME"
        exit 1
    fi
done
